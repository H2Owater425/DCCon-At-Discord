...
def getFixedTag(keyword, tag):
	originList = ['RCT', '롤코타', '게비스콘', '계란', '달걀콘', '눈물콘', '냥이', '동숲', '레바콘', '마울어', '마크', '문풍당당', '이풍당당', '예풍당당', '볼트보이콘', '북돼지', '이북리더', '심의콘', '시스카', '젤다', '짜잔콘', '샌즈', '캐피탈리즘호', '카티아', '컴', '컴갤콘', '토드콘', '포켓몬']
	fixedList = ['롤러코스터타이쿤', '롤러코스터타이쿤', '개비스콘', '껍질미리깐달걀', '껍질미리깐달걀', '눈물', '고양이', '동물의숲', '레바', '마이너울트라어드벤처', '마인크래프트', '당당', '당당', '당당', '볼트보이', '김정은', '김정은', '심의', '시티즈스카이라인', '젤다의전설', '제프카플란', '언더테일', '루세티아', '카티아마나간', '컴퓨터', '컴퓨터갤러리', '토드하워드', '포켓몬스터']
...
while True:
	...
	if('파누' in funzinnuFDFJSON[i]['tags'] or '미지정' in funzinnuFDFJSON[i]['tags']):
		...
		j = 0
		while True:
			if(j == len(funzinnuFDFJSON[i]['keywords'])):
				break
			if(re.fullmatch(re.compile('(갤주.*)'), funzinnuFDFJSON[i]['keywords'][j]) != None):
				del funzinnuFDFJSON[i]['keywords'][j]
				j = 0
			else:
				j += 1
		if(len(funzinnuFDFJSON[i]['keywords']) < 1):
			del funzinnuFDFJSON[i]
			i = 0
		elif(len(funzinnuFDFJSON[i]['tags']) < 1):
			del funzinnuFDFJSON[i]
			i = 0
		else:
			i += 1
	...
	if('텔크' in telkODFJSON[i]['tags']):
		...
		while True:
			if(j == len(telkODFJSON[i]['keywords'])):
				break
			if(re.fullmatch(re.compile('(((.*telk.*)|(.*텔.*)|(.*텔크.*))(?<!텔)$)'), telkODFJSON[i]['keywords'][j]) != None):
				del telkODFJSON[i]['keywords'][j]
				j = 0
			elif(re.fullmatch(re.compile('(갤주.*)'), telkODFJSON[i]['keywords'][j]) != None):
				del telkODFJSON[i]['keywords'][j]
				j = 0
			else:
				j += 1
...
while True:
	...
	if('여까티콘' in yeokkaODFJSON[i]['tags'] or '움짤' in yeokkaODFJSON[i]['tags']):
		...
		j = 0
		while True:
			if(j == len(yeokkaODFJSON[i]['keywords'])):
				break
			if(re.fullmatch(re.compile('(갤주.*)'), yeokkaODFJSON[i]['keywords'][j]) != None):
				del yeokkaODFJSON[i]['keywords'][j]
				j = 0
			else:
				j += 1
		if(len(yeokkaODFJSON[i]['keywords']) < 1):
			del yeokkaODFJSON[i]
			i = 0
		elif(len(yeokkaODFJSON[i]['tags']) < 1):
			del yeokkaODFJSON[i]
			i = 0
		else:
			i += 1
...
while True:
	...
	if('움짤' in poopkiODFJSON[i]['tags']):
		...
		j = 0
		while True:
			if(j == len(poopkiODFJSON[i]['keywords'])):
				break
			if(re.fullmatch(re.compile('(갤주.*)'), poopkiODFJSON[i]['keywords'][j]) != None):
				del poopkiODFJSON[i]['keywords'][j]
				j = 0
			else:
				j += 1
		if(len(poopkiODFJSON[i]['keywords']) < 1):
			del poopkiODFJSON[i]
			i = 0
		elif(len(poopkiODFJSON[i]['tags']) < 1):
			del poopkiODFJSON[i]
			i = 0
		else:
			i += 1
...
for i in range(len(searchResult)):
	dcConNameList.append(searchResult[i]['id'])

for i in range(len(dcConNameList)):
	if(dcConNameList[i].endswith('1')):
		if(not dcConNameList[i][:-1] in dcConNameList):
			dcConAddName = dcConNameList[i][:-1]
			cursor.execute(f'INSERT IGNORE INTO `dccon` (`id`, `tag`, `url`, `last_update`) values (\'{dcConAddName}\', \'{searchResult[i]["tag"]}\', \'{searchResult[i]["url"]}\', {searchResult[i]["last_update"]})')
	elif(dcConNameList[i]+'2' in dcConNameList and not dcConNameList[i]+'1' in dcConNameList):
		dcConAddName = dcConNameList[i]+'1'
		cursor.execute(f'INSERT IGNORE INTO `dccon` (`id`, `tag`, `url`, `last_update`) values (\'{dcConAddName}\', \'{searchResult[i]["tag"]}\', \'{searchResult[i]["url"]}\', {searchResult[i]["last_update"]})')
